...
export default function Revise() {
  ...
  const [mode, setMode] = useState('learn') // 'learn' | 'revise'
  ...
  useEffect(() => {
    ...
    api.get(`/api/cards?languageId=${selectedLang}&limit=200&mode=learn`).then(r => {
      setLearnCount((r.data.cards || r.data).length)
    })
    api.get(`/api/cards?languageId=${selectedLang}&limit=200&mode=revise`).then(r => {
      setReviseCount((r.data.cards || r.data).length)
    })
  }, [selectedLang])

  const availableCount = mode === 'learn' ? learnCount : reviseCount
    ...
    <div className="min-h-[calc(100vh-72px)] bg-[#f8fafc] dark:bg-[#070f1c] px-6 md:px-10 py-12">
      <div className="max-w-lg mx-auto">
        ...
        <div className="p-7 rounded-2xl border border-gray-200 dark:border-white/[0.08] bg-white dark:bg-[#0c1a2e] shadow-sm space-y-6">
          ...
          {/* Mode selector */}
          <div>
            <label className="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">Session type</label>
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => setMode('learn')}
                className={`py-3 px-4 rounded-xl border text-sm font-semibold transition-all text-left ${mode === 'learn' ? 'border-teal-500 bg-teal-50 dark:bg-teal-900/30 text-teal-800 dark:text-teal-200' : 'border-gray-200 dark:border-white/10 text-gray-700 dark:text-gray-300 hover:border-gray-300 dark:hover:border-white/20'}`}>
                <p>Learn</p>
                <p className="text-xs font-normal mt-0.5 opacity-70">New vocabulary</p>
              </button>
              <button onClick={() => setMode('revise')}
                className={`py-3 px-4 rounded-xl border text-sm font-semibold transition-all text-left ${mode === 'revise' ? 'border-sky-500 bg-sky-50 dark:bg-sky-900/30 text-sky-800 dark:text-sky-200' : 'border-gray-200 dark:border-white/10 text-gray-700 dark:text-gray-300 hover:border-gray-300 dark:hover:border-white/20'}`}>
                <p>Revise</p>
                <p className="text-xs font-normal mt-0.5 opacity-70">Due for review</p>
              </button>
            </div>
          </div>
          ...
          {selectedLang && availableCount === 0 && (
            <p className="text-sm text-center text-gray-500 dark:text-gray-400 py-1">
              {mode === 'learn'
                ? 'No new cards — add more vocabulary to your language first.'
                : 'Nothing due for revision yet — check back later.'}
            </p>
          )}
          ...
          <button onClick={startSession} disabled={!selectedLang || availableCount === 0}
            ...
            Start {mode === 'learn' ? 'learning' : 'revision'} →
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="min-h-[calc(100vh-72px)] bg-[#f8fafc] dark:bg-[#070f1c] flex items-center justify-center px-6">
      <div className="text-center max-w-sm">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-8">
          {mode === 'learn' ? 'Missed cards will come back in your next revision session.' : 'Missed cards are rescheduled for later.'}
        </p>
