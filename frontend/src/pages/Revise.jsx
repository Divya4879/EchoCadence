...
const WRONG_MESSAGES = ["Not there yet — you'll get it.", "Almost. Keep going.", "It'll click soon.", "No rush — try again later."]
const RIGHT_MESSAGES  = ["Locked in.", "That's the one.", "Clean.", "Solid memory.", "You knew that."]
const RETRY_MESSAGES  = ["Second chance — you've got this.", "One more look.", "Almost there."]
...
export default function Revise() {
  ...
  const [retrySet, setRetrySet] = useState([])  // cards missed first time
  ...
  useEffect(() => {
    ...
    api.get(`/api/cards?languageId=${selectedLang}&limit=200`).then(r => {
  ...
  async function startSession() {
    if (!selectedLang) return
    const { data } = await api.get(`/api/cards?languageId=${selectedLang}&limit=200`)
  ...
  const progress = totalCards ? ((idx + 1) / totalCards) * 100 : 0

  if (stage === 'setup') return (
    <div className="min-h-[calc(100vh-72px)] bg-[#f8fafc] dark:bg-[#070f1c] px-6 md:px-10 py-12">
      <div className="max-w-lg mx-auto">
        <Breadcrumbs />
        <p className="text-xs font-bold uppercase tracking-widest text-teal-600 dark:text-teal-400 mb-2">Flashcards</p>
        <h1 className="text-3xl font-serif font-medium text-gray-900 dark:text-white mb-10">Learn & Revise</h1>
        <div className="p-7 rounded-2xl border border-gray-200 dark:border-white/[0.08] bg-white dark:bg-[#0c1a2e] shadow-sm space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {[10, 20, 30, 50].filter(n => !selectedLang || n <= availableCount || n === Math.min(...[10,20,30,50].filter(x => x <= availableCount)) || availableCount < 10).map(n => {
                const effective = Math.min(n, availableCount || n)
                return (
                  <button key={n} onClick={() => setSessionSize(n)}
                    className={`flex-1 py-2 rounded-xl border text-sm font-medium transition-all ${sessionSize === n ? 'border-teal-500 bg-teal-50 dark:bg-teal-900/30 text-teal-800 dark:text-teal-200' : 'border-gray-200 dark:border-white/10 text-gray-700 dark:text-gray-300 hover:border-gray-300 dark:hover:border-white/20'} ${selectedLang && n > availableCount ? 'opacity-40 cursor-not-allowed' : ''}`}
          ...
          {selectedLang && availableCount === 0 && (
            <p className="text-sm text-center text-gray-500 dark:text-gray-400 py-2">
              Nothing due right now — you're all caught up. Check back later or add more vocabulary.
            </p>
          )}
  ...
  if (stage === 'done') return (
    <div className="min-h-[calc(100vh-72px)] bg-[#f8fafc] dark:bg-[#070f1c] flex items-center justify-center px-6">
      <div className="text-center max-w-sm">
        ...
        <h2 className="text-2xl font-serif font-medium text-gray-900 dark:text-white mb-2">Session done.</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-8">Any cards you missed have been queued for your next session.</p>
        <div className="flex gap-3 justify-center">
          <button onClick={() => setStage('setup')} className="px-6 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 text-sm font-semibold text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">New session</button>
          <button onClick={() => navigate('/progress')} className="px-6 py-2.5 rounded-xl bg-brand-gradient text-white text-sm font-semibold hover:opacity-90 glow">View progress →</button>
        </div>
      </div>
    </div>
  )
      ...
      <div className="max-w-xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <span className="text-sm font-medium text-gray-500 dark:text-gray-400">
            {isRetryRound ? `Retry · ${idx + 1}/${totalCards}` : `${idx + 1} / ${totalCards}`}
          </span>
        ...
        <div style={{ perspective: '1200px' }}>
          <div
            onClick={() => !gotRight && setFlipped(f => !f)}
            className={`relative w-full cursor-pointer transition-transform duration-500 ${celebrate ? 'scale-[1.02]' : ''}`}
          ...
          >
            {/* Front */}
            <div className="absolute inset-0 rounded-2xl border border-gray-200 dark:border-white/[0.08] bg-white dark:bg-[#0c1a2e] shadow-xl shadow-gray-200/40 dark:shadow-black/30 flex flex-col items-center justify-center p-10 text-center"
            ...
            {/* Back */}
            <div className={`absolute inset-0 rounded-2xl border bg-white dark:bg-[#0c1a2e] shadow-xl shadow-gray-200/40 dark:shadow-black/30 flex flex-col items-center justify-center p-10 text-center transition-colors ${celebrate ? 'border-teal-400 dark:border-teal-500/60' : 'border-teal-200 dark:border-teal-700/40'}`}
